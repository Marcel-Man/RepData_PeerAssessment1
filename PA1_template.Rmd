...
Next, we compute the average number of steps over each 5 mins inteval and plot the resulting time series. The interval at which the maximum number of steps is recorded is also calculated.

```{r}
steps_by_interval <- tapply(act_df$steps, strftime(act_df$datetime, format="%H:%M"), FUN=sum, na.rm=TRUE)
...
steps_by_hour <- tapply(act_df$steps, strftime(act_df$datetime, format="%Y-%m-%d %H"), FUN=sum, na.rm=T)
